...
RUN apt-get update && apt-get install -y  git-core libcurl4-openssl-dev libgit2-dev libglpk-dev libgmp-dev libicu-dev libssl-dev libxml2-dev make pandoc pandoc-citeproc zlib1g-dev && rm -rf /var/lib/apt/lists/*
...
RUN Rscript -e 'remotes::install_version("scholar",upgrade="never", version = "0.2.2")'
...
EXPOSE 80
CMD R -e "options('shiny.port'=80,shiny.host='0.0.0.0');indiceH::run_app()"
